...
if ( ! function_exists( '<%- pkgNameLowerCase -%>_pagination' ) ) :
	function <%- pkgNameLowerCase -%>_pagination() {
		global $wp_query;
...
if ( ! function_exists( '<%- pkgNameLowerCase -%>_get_the_comments_pagination' ) ) :
	function <%- pkgNameLowerCase -%>_get_the_comments_pagination( $args = array() ) {
		$navigation = '';
...
// Custom Comments Pagination.
if ( ! function_exists( '<%- pkgNameLowerCase -%>_the_comments_pagination' ) ) :
	function <%- pkgNameLowerCase -%>_the_comments_pagination( $args = array() ) {
		echo <%- pkgNameLowerCase -%>_get_the_comments_pagination( $args );
	}
endif;
...
if ( ! function_exists( '<%- pkgNameLowerCase -%>_menu_fallback' ) ) :
	function <%- pkgNameLowerCase -%>_menu_fallback() {
		echo '<div class="alert-box secondary">';
		/* translators: %1$s: link to menus, %2$s: link to customize. */
		printf(
			__( 'Please assign a menu to the primary menu location under %1$s or %2$s the design.', '<%- textDomain -%>' ),
			/* translators: %s: menu url */
			sprintf(
				__( '<a href="%s">Menus</a>', '<%- textDomain -%>' ),
				get_admin_url( get_current_blog_id(), 'nav-menus.php' )
			),
			/* translators: %s: customize url */
			sprintf(
				__( '<a href="%s">Customize</a>', '<%- textDomain -%>' ),
...
if ( ! function_exists( '<%- pkgNameLowerCase -%>_active_nav_class' ) ) :
	function <%- pkgNameLowerCase -%>_active_nav_class( $classes, $item ) {
		if ( $item->current == 1 || $item->current_item_ancestor == true ) {
			$classes[] = 'is-active';
		}
		return $classes;
	}
	add_filter( 'nav_menu_css_class', '<%- pkgNameLowerCase -%>_active_nav_class', 10, 2 );
endif;
 ...
 */
if ( ! function_exists( '<%- pkgNameLowerCase -%>_active_list_pages_class' ) ) :
	function <%- pkgNameLowerCase -%>_active_list_pages_class( $input ) {
	...
	add_filter( 'wp_list_pages', '<%- pkgNameLowerCase -%>_active_list_pages_class', 10, 2 );
endif;
...
if ( ! function_exists( '<%- pkgNameLowerCase -%>_mobile_menu_id' ) ) :
	function <%- pkgNameLowerCase -%>_mobile_menu_id() {
 ...
 */
if ( ! function_exists( '<%- pkgNameLowerCase -%>_gallery' ) ) :
	function <%- pkgNameLowerCase -%>_gallery($attr) {
	...
	add_shortcode('gallery', '<%- pkgNameLowerCase -%>_gallery');
endif;
